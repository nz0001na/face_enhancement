...
%creat result file
dst = 'F:\zn1\znMCM\MsCeleb1M_code\frontalize_output\qa_low_sys\';
    if ~exist(dst, 'dir')
        mkdir('F:\zn1\znMCM\MsCeleb1M_code\frontalize_output\qa_low_sys\');  
    end
    
%poselist = 'F:\zn1\znMCM\MsCeleb1M_code\code_openface\pose\low\';
...
%       hold off; title('Query photo with detections overlaid');
       ...
       imwrite(frontal_sym, [dstq files(j).name]);
    end
