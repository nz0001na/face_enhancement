...
% low vs. high--  FaceNet,  CenterLoss,  lightCNN,  
probe_path = 'F:\zn1\znMCM\MsCeleb1M_code\code_deep_fr\feature_output\VGGface\0126_V2\low';
d = dir(probe_path);
isub = [d(:).isdir]; 
nameFolds = {d(isub).name}';
nameFolds(ismember(nameFolds,{'.','..'})) = [];
gallery_path = 'F:\zn1\znMCM\MsCeleb1M_code\code_deep_fr\feature_output\VGGface\0126_V2\high';

% destination
dst = '../pairs_features/VGGface/0126_V2_LvsH/';
if ~exist(dst,'dir')
    mkdir(['../pairs_features/VGGface/0126_V2_LvsH/']);
end
% create pairs
...
%     end
    ...
    %% create pairs
    for i=1:size(P1,1) % probe name and feature
%         p_feat = ffeature_Probe(i,:);          
        pname = P1(i);      
        p_name = char(pname(1,1));

       for j=1:size(G1,1)  % gallery name and feature
%            g_feat = ffeature_Gallery(j,:);          
           gname = G1(j);      
           g_name = char(gname(1,1));
     
           fprintf(fpos_pairs, '%s,%s,%d,%s,%s,%d\n', p_name, subject_id_probe,i, g_name, subject_id_gallery,j); 
