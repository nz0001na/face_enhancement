...
% destination
dst = '../pairs_features/FaceNet/0126_V_LvsH/negative/';
if ~exist(dst,'dir')
    mkdir(['../pairs_features/FaceNet/0126_V_LvsH/negative/']);
end

for n=2:sum_prob % low faces ---probe
    fprintf('%d/%d %s\n', n, length(nameFolds), nameFolds{n});
    subject_id_probe = nameFolds{n};   
    
    dstq = [dst subject_id_probe '/'];
        if ~exist(dstq,'dir')
        mkdir([dst subject_id_probe '/']);
    end

    % create pairs
    fneg_pairs = fopen([dstq subject_id_probe '_neg_pairs.csv'], 'w');
    neg_probe_feature = [];
    neg_gallery_feature = [];
    ...
    fclose(fnames_Probe);
    csvwrite([dstq subject_id_probe '_neg_low_feature.csv'], neg_probe_feature);
    csvwrite([dstq subject_id_probe '_neg_high_feature.csv'], neg_gallery_feature);
    fclose(fneg_pairs);
