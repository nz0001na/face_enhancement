...
pathFolder = 'F:\zn1\znMCM\MsCeleb1M_code\frontalize_output\low_landmark_xyz_rgb';
d = dir(pathFolder);
...
%creat result folder
dstq = '../cropped_output/frontalization/low_xyz_rgb/';  
    if ~exist(dstq, 'dir')
        mkdir('../cropped_output/frontalization/low_xyz_rgb/');  
    end
    
landmarkdir = 'F:\zn1\znMCM\MsCeleb1M_code\code_openface\frontalization_landmark_xyz_rgb\low_pose\';
...
for j = 1:length(nameFolds)
    ...
    %destination
    dstqq = [dstq subject_id '/'];
    if ~exist(dstqq, 'dir')
        mkdir([dstq subject_id '/']);  
    end
