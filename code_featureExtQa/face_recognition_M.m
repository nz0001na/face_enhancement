...
pathFolder = 'F:\zn1\znMCM\MsCeleb1M_code\datasets_facescrub\statistic_rqs_3f\shorten_QAoutput';
d = dir(pathFolder);
...
%creat result folder
dst = '../facerecognitionM/facescrub_gabor/';
    if ~exist(dst, 'dir')
        mkdir(['../facerecognitionM/facescrub_gabor/']);
        mkdir(['../facerecognitionM/facescrub_gabor/ProbeScore/']);
    end
...
for n = 1:sum_sub     
    ...
    fsubject_s_Probe = fopen([pathFolder '/' subject_id_probe '/' subject_id_probe '_middle_name.csv'], 'r');
    ...
    subject_f_Probe = csvread([pathFolder '/' subject_id_probe '/' subject_id_probe '_middle_feature.csv']); 
    ...
    for l=1:size(subject_f_Probe,1)
        ...
        for m = 1:sum_sub
            ...
            subject_f_Gallery = csvread([pathFolder '/' subject_id_gallery '/' subject_id_gallery '_high_feature.csv']); 
